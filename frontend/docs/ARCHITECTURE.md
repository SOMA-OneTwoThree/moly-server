...
## 8. 개선 백로그 (기록만 — 이번 문서화에서는 코드 미변경)

- **스타일링 2종 혼재**: 인라인 스타일(`layout.tsx`, `page.tsx`, `auth/callback`)과 CSS Module(`policy`/`support`)이 공존. 전역 CSS/CSS 변수는 없음.
- **디자인 토큰 중복**: 같은 팔레트가 파일마다 raw hex로 재기재. 특히 primary blue가 **2개**(`#4285f4` 인라인 vs `#6ea8fe` CSS Module), 보더 그레이도 `#23262d`/`#2a2c33` 혼용. → `globals.css` CSS 변수 또는 테마 모듈로 단일화 여지.
- **공용 컴포넌트/헬퍼 부재**: `components/` 없음, `lib/`은 `supabaseClient.ts` 하나. Bearer 토큰 추출 + `API_BASE` fetch가 `page.tsx`에 인라인 → 인증 호출이 늘면 `apiFetch` 헬퍼로 추출 여지.
